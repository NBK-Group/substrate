...
{
	fn coco_seal(signature: Signature) -> Self {
		DigestItem::Seal(COCO_ENGINE_ID, signature.encode())
	}

	fn as_coco_seal(&self) -> Option<Signature> {
		self.seal_try_to(&COCO_ENGINE_ID)
	}

	fn coco_pre_digest(slot: Slot) -> Self {
		DigestItem::PreRuntime(COCO_ENGINE_ID, slot.encode())
	}

	fn as_coco_pre_digest(&self) -> Option<Slot> {
		self.pre_runtime_try_to(&COCO_ENGINE_ID)
	}
}
