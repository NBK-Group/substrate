...
pub const INHERENT_IDENTIFIER: InherentIdentifier = *b"cocoslot";

/// The type of the Aura inherent.
pub type InherentType = sp_consensus_slots::Slot;

/// Auxiliary trait to extract Aura inherent data.
pub trait CocoInherentData {
	/// Get aura inherent data.
	fn coco_inherent_data(&self) -> Result<Option<InherentType>, Error>;
	/// Replace aura inherent data.
	fn coco_replace_inherent_data(&mut self, new: InherentType);
}

impl CocoInherentData for InherentData {
	fn coco_inherent_data(&self) -> Result<Option<InherentType>, Error> {
