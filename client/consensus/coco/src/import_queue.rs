...
use crate::{coco_err, authorities, find_pre_digest, slot_author, AuthorityId, Error};
...
use sp_consensus_coco::{
	digests::CompatibleDigestItem, inherents::CocoInherentData, CocoApi, ConsensusLog,
	COCO_ENGINE_ID,
};
	...
	P::Public: Encode + Decode + PartialEq + Clone,
{
	...
	let sig = seal.as_coco_seal().ok_or_else(|| coco_err(Error::HeaderBadSeal(hash)))?;
...
pub struct CocoVerifier<C, P, CAW, CIDP> {
	client: Arc<C>,
...
impl<C, P, CAW, CIDP> CocoVerifier<C, P, CAW, CIDP> {
	pub(crate) fn new(
		client: Arc<C>,
		create_inherent_data_providers: CIDP,
		can_author_with: CAW,
...
impl<B: BlockT, C, P, CAW, CIDP> Verifier<B> for CocoVerifier<C, P, CAW, CIDP>
where
	C: ProvideRuntimeApi<B> + Send + Sync + sc_client_api::backend::AuxStore + BlockOf,
	C::Api: BlockBuilderApi<B> + CocoApi<B, AuthorityId<P>> + ApiExt<B>,
		...
		match checked_header {
			CheckedHeader::Checked(pre_header, (slot, seal)) => {
				...
				if let Some(inner_body) = block.body.take() {
					...
					inherent_data.coco_replace_inherent_data(slot);
				...
				trace!(target: "coco", "Checked {:?}; importing.", pre_header);
				telemetry!(
					self.telemetry;
					CONSENSUS_TRACE;
					"coco.checked_and_importing";
			...
			CheckedHeader::Deferred(a, b) => {
				debug!(target: "coco", "Checking {:?} failed; {:?}, {:?}.", hash, a, b);
				telemetry!(
					self.telemetry;
					CONSENSUS_DEBUG;
					"coco.header_too_far_in_future";
					"hash" => ?hash,
					"a" => ?a,
					"b" => ?b,
				);
...
where
	Block: BlockT,
	C::Api: BlockBuilderApi<Block> + CocoApi<Block, AuthorityId<P>> + ApiExt<Block>,
	C: 'static
...
) -> CocoVerifier<C, P, CAW, CIDP> {
	CocoVerifier::<_, P, _, _>::new(
		client,
		create_inherent_data_providers,
		can_author_with,
		check_for_equivocation,
