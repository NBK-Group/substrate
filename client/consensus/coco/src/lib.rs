...
pub use import_queue::{
	build_verifier, import_queue, CocoVerifier, BuildVerifierParams, CheckForEquivocation,
	ImportQueueParams,
};
pub use sc_consensus_slots::SlotProportion;
pub use sp_consensus::SyncOracle;
pub use sp_consensus_coco::{
	digests::CompatibleDigestItem,
	inherents::{InherentDataProvider, InherentType as CocoInherent, INHERENT_IDENTIFIER},
	CocoApi, ConsensusLog, SlotDuration, COCO_ENGINE_ID,
};
...
pub struct StartCocoParams<C, SV, SC, I, PF, SO, L, CIDP, BS, CAW> {
...
pub fn start_coco<P, SV, B, C, SC, I, PF, SO, L, CIDP, BS, CAW, Error>(
	StartCocoParams {
		slot_duration,
		client,
		server,
		...
		telemetry,
	}: StartCocoParams<C, SV, SC, I, PF, SO, L, CIDP, BS, CAW>,
...
{
	let worker = build_coco_worker::<P, SV, _, _, _, _, _, _, _, _>(BuildCocoWorkerParams {
		client: client.clone(),
		server: server.clone(),
		block_import,
		proposer_factory,
...
pub struct BuildCocoWorkerParams<C, SV, I, PF, SO, L, BS> {
	/// The client to interact with the chain.
	pub client: Arc<C>,
	/// The server
	pub server: Arc<SV>,
...
pub fn build_coco_worker<P, SV, B, C, PF, I, SO, L, BS, Error>(
	BuildCocoWorkerParams {
		client,
		server,
		block_import,
		proposer_factory,
		...
		telemetry,
		force_authoring,
	}: BuildCocoWorkerParams<C, SV, I, PF, SO, L, BS>,
...
struct CocoWorker<C,  SV, E, I, P, SO, L, BS> {
	client: Arc<C>,
	server: Arc<SV>,
	block_import: I,
	env: E,
...
impl<B, C, SV, E, I, P, Error, SO, L, BS> sc_consensus_slots::SimpleSlotWorker<B>
	for CocoWorker<C, SV, E, I, P, SO, L, BS>
where
	B: BlockT,
	...
	Error: std::error::Error + Send + From<sp_consensus::Error> + 'static,
{
	...
	fn pre_digest_data(&self, slot: Slot, _claim: &Self::Claim) -> Vec<sp_runtime::DigestItem> {
		vec![<DigestItem as CompatibleDigestItem<P::Signature>>::coco_pre_digest(slot)]
	}
	...
	> {
		...
		let signature_digest_item =
			<DigestItem as CompatibleDigestItem<P::Signature>>::coco_seal(signature);
...
fn coco_err<B: BlockT>(error: Error<B>) -> Error<B> {
	debug!(target: "coco", "{}", error);
	error
}
...
pub fn find_pre_digest<B: BlockT, Signature: Codec>(header: &B::Header) -> Result<Slot, Error<B>> {
	...
	for log in header.digest().logs() {
		trace!(target: "coco", "Checking log {:?}", log);
		match (CompatibleDigestItem::<Signature>::as_coco_pre_digest(log), pre_digest.is_some()) {
			(Some(_), true) => Err(coco_err(Error::MultipleHeaders))?,
			(None, _) => trace!(target: "coco", "Ignoring digest not meant for us"),
			(s, false) => pre_digest = s,
		}
	}
	pre_digest.ok_or_else(|| coco_err(Error::NoDigestFound))
}
...
mod tests {
	...
	type AuraVerifier = import_queue::CocoVerifier<
		PeersFullClient,
		AuthorityPair,
		AlwaysCanAuthor,
		Box<
	...
	fn authoring_blocks() {
		...
		for (peer_id, key) in peers {
			...
			aura_futures.push(
				start_coco::<AuthorityPair, _, _, _, _, _, _, _, _, _, _, _>(StartCocoParams {
					slot_duration,
